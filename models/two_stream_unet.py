...
class unet_vgg16(nn.Module):
    ...
    def __init__(self, inp_ch, kernel_size=3, skip=True):
        ...
        self.conv1 = nn.Conv2d(64  * 2, 64,  1, padding="same")
        self.conv2 = nn.Conv2d(128 * 2, 128, 1, padding="same")
        self.conv3 = nn.Conv2d(256 * 2, 256, 1, padding="same")
        self.conv4 = nn.Conv2d(512 * 2, 512, 1, padding="same")
        self.conv5 = nn.Conv2d(512 * 2, 512, 1, padding="same")
    ...
    def forward(self, inp):
        ...
        d1 = torch.cat([empty_bg_feats[0], curr_fr_feats[0]], dim = 1)
        d1 = self.conv1(d1)
        d2 = torch.cat([empty_bg_feats[1], curr_fr_feats[1]], dim = 1)
        d2 = self.conv2(d2)
        d3 = torch.cat([empty_bg_feats[2], curr_fr_feats[2]], dim = 1)
        d3 = self.conv3(d3)
        d4 = torch.cat([empty_bg_feats[3], curr_fr_feats[3]], dim = 1)
        d4 = self.conv4(d4)
        d5 = torch.cat([empty_bg_feats[4], curr_fr_feats[4]], dim = 1)
        d5 = self.conv5(d5)
